...
| oauth2.existingSecret | string | "" | Name of a pre-existing Secret containing oauth2-proxy credentials. Required when oauth2.enabled=true. The secret must contain OAUTH2_PROXY_COOKIE_SECRET and, unless publicClient=true, OAUTH2_PROXY_CLIENT_ID and OAUTH2_PROXY_CLIENT_SECRET. When provisionClient=true, the Keycloak operator reads the client secret from this secret and registers it with Keycloak, but does not write into it. You must ensure the secret exists before deploying. The recommended way to provision this secret is via templates.sealedSecrets in the KvalitetsIT templates chart (https://github.com/KvalitetsIT/helm-templates-chart). |
| oauth2.realm | string | "" | Keycloak realm name. Always used together with issuerUrl to construct the OIDC issuer URL. When provisionClient=true, also provisions a KeycloakClient CRD (clusterRealmRef). When provisionClient=false (default), only the issuer URL is derived. |
| oauth2.provisionClient | bool | false | When true, provisions a KeycloakClient CRD to register the application in Keycloak. The chart does not generate the Secret - you must pre-create it via oauth2.existingSecret with all required keys. The operator reads the client secret from it and registers the value with Keycloak, but does not write into it. See oauth2.existingSecret for the required keys.  Set to false to manage the Keycloak client externally while still deriving the OIDC issuer URL from realm and issuerUrl. **Requires the Keycloak Operator** to be installed in the cluster - see the README's [Keycloak client provisioning](#keycloak-client-provisioning) section. |
| oauth2.clientDefinition | object | `{"attributes":{"post.logout.redirect.uris":"+"},"defaultClientScopes":null,"directAccessGrantsEnabled":false,"enabled":true,"publicClient":false,"serviceAccountsEnabled":false,"standardFlowEnabled":true,"webOrigins":["+"]}` | Keycloak ClientRepresentation fields passed to the KeycloakClient spec.definition. clientId and redirectUris are always auto-derived. Setting publicClient: true skips clientSecretRef on the KeycloakClient and omits OAUTH2_PROXY_CLIENT_SECRET from the required secret keys. |
...
- when `oauth2.enabled=true`: `oauth2.realm`, `oauth2.issuerUrl`, and `oauth2.existingSecret` are required
...
Set `oauth2.provisionClient=true` to have the chart create a `KeycloakClient` CRD. You must supply
the oauth2-proxy credentials via `oauth2.existingSecret` - the chart does not generate the Secret.

The secret must exist in the cluster before deploying and contain the following keys:
