...
| route.type | string | HTTPRoute | Route type. HTTPRoute terminates TLS at the gateway (cert-manager manages the certificate). TLSRoute passes TLS through to the backend — the application owns the certificate. Note: TLSRoute uses gateway.networking.k8s.io/v1alpha2 (experimental channel). |
